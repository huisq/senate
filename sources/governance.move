module overmind::governance {
    ...
    */
    fun init_module(admin: &signer) {
        ...
        let new_map = simple_map::create();
        simple_map::add(&mut new_map, governance_responsibility_key, resource_cap);
        let governance_responsibility = GovernanceResponsibility{
            signer_caps: new_map
        };

        // TODO: Create the ApprovedExecutionHashes with an empty map
        let approved_exe_hash = ApprovedExecutionHashes{
        ...
        let governance_events = GovernanceEvents{
            ...
            updated_governance_config_events: account::new_event_handle<UpdateGovernanceConfigEvent>(admin),
            update_governance_responsibility_events: account::new_event_handle<UpdateGovernanceResponsibilityEvent>(admin),
            add_approved_execution_hash_events: account::new_event_handle<AddApprovedExecutionHashEvent>(admin),
            resolve_proposal_events: account::new_event_handle<ResolveProposalEvent>(admin)
        };

        // TODO: Move the 4 new global objects to the resource account
        move_to<GovernanceConfig>(&resource_signer,governance_config);
        move_to<GovernanceResponsibility>(&resource_signer,governance_responsibility);
        move_to<ApprovedExecutionHashes>(&resource_signer,approved_exe_hash);
        move_to<GovernanceEvents>(&resource_signer,governance_events);
    }
    ...
    */
    public entry fun create_proposal(
        ...
        let token_address = get_governenace_token_address(signer::address_of(proposer));
        check_if_address_is_an_object(token_address);
        ...
        let token_obj = object::address_to_object<GovernanceToken>(token_address);
        check_if_account_can_create_proposal(token_obj);
        ...
        let gov_events = borrow_global_mut<GovernanceEvents>(get_resource_account_address());
        event::emit_event<CreateProposalEvent>(
            &mut gov_events.create_proposal_events,
            CreateProposalEvent{
                proposer: signer::address_of(proposer),
                proposer_governance_token_address: token_address,
                proposal_id,
                execution_hash,
                proposal_metadata
            });
    ...
    */
    public entry fun vote (
        ...
        //
        // HINT: Use the check_if_address_is_an_object function below
        let token_address = get_governenace_token_address(signer::address_of(voter));
        check_if_address_is_an_object(token_address);
        ...
        check_if_voter_has_not_voted_on_proposal(proposal_id, token_obj);
        ...
        let gov_token = borrow_global_mut<GovernanceToken>(token_address);
        property_map::add_typed(&gov_token.property_mutator_ref,generate_proposal_id_key(proposal_id),proposal_id);

        // TODO: Fetch the voter's voting power and execute the vote
        //
        // HINT: Use the vote_internal function below
        let voting_power = property_map::read_u64<GovernanceToken>(&token_obj,&string::utf8(b"VOTING_POWER"));
        vote_internal(proposal_id, voting_power, should_pass);

        // TODO: Emit a VoteEvent
        let gov_events = borrow_global_mut<GovernanceEvents>(get_resource_account_address());
        event::emit_event<VoteEvent>(
            &mut gov_events.vote_events,
            VoteEvent{
                proposal_id,
    ...
    */
    public fun update_governance_config(
        ...
        let gov_config = borrow_global_mut<GovernanceConfig>(get_resource_account_address());

        // TODO: Record the old config values
        let old_minimum_voting_threshold = gov_config.minimum_voting_threshold;
        let old_voting_duration_seconds = gov_config.voting_duration_seconds;

        // TODO: Update the config with the new values
        gov_config.minimum_voting_threshold = minimum_voting_threshold;
        gov_config.voting_duration_seconds = voting_duration_seconds;

        // TODO: Emit the UpdateGovernanceConfigEvent
        let gov_events = borrow_global_mut<GovernanceEvents>(get_resource_account_address());
        event::emit_event<UpdateGovernanceConfigEvent>(
            &mut gov_events.updated_governance_config_events,
    ...
    */
    public fun update_governance_responsibility(
        ...
        let gov_resp = borrow_global_mut<GovernanceResponsibility>(get_resource_account_address());
        let governance_responsibility_key = GovernanceResponsibilityKey{key, description};
        let (signer_key, signer_description) = simple_map::upsert(&mut gov_resp.signer_caps, governance_responsibility_key, signer_cap);
        let old_signer_key;
        let old_signer_description;
        if(option::is_none(&signer_key)){
            old_signer_key = option::none();
            old_signer_description = option::none();
        }else{
            old_signer_key = option::some(string_utils::to_string(option::borrow(&signer_key)));
            old_signer_description = option::some(string_utils::to_string(option::borrow(&signer_description)));
        };
        // TODO: Emit the UpdateGovernanceResponsibilityEvent
        let gov_events = borrow_global_mut<GovernanceEvents>(get_resource_account_address());
        event::emit_event<UpdateGovernanceResponsibilityEvent>(
            &mut gov_events.update_governance_responsibility_events,
    ...
    */
    public entry fun add_approved_script_hash(proposal_id: u64) acquires ApprovedExecutionHashes, GovernanceEvents {
        ...
        let approved_exe_hash = borrow_global_mut<ApprovedExecutionHashes>(get_resource_account_address());
        simple_map::upsert(
            &mut approved_exe_hash.hashes,
            proposal_id,
            voting::get_execution_hash<GovernanceProposalType>(get_resource_account_address(),proposal_id)
        );

        // Emit the AddApprovedExecutionHashEvent
        let gov_events = borrow_global_mut<GovernanceEvents>(get_resource_account_address());
        event::emit_event<AddApprovedExecutionHashEvent>(
            &mut gov_events.add_approved_execution_hash_events,
    ...
    */
    public fun resolve(
        ...
        voting::resolve<GovernanceProposalType>(get_resource_account_address(),proposal_id);
        ...
        let gov_events = borrow_global_mut<GovernanceEvents>(get_resource_account_address());
        event::emit_event<ResolveProposalEvent>(
            &mut gov_events.resolve_proposal_events,
    ...
    */
    public fun resolve_multi_step_proposal(
        ...
        voting::resolve_proposal_v2<GovernanceProposalType>(get_resource_account_address(), proposal_id, next_execution_hash);
    ...
    */
    public fun mint_governance_token(
        ...
        let token_name = generate_governance_token_name(soul_bound_to);

        // TODO: Generate the name for this new governance token
        // 
        // HINT: use the generate_governance_token_name function below
        let token_description = generate_governance_token_description(soul_bound_to);
        ...
        object::transfer_raw(creator, object::address_from_constructor_ref(&token_const_ref),soul_bound_to);
        ...
        let prop_keys = vector[
            string::utf8(b"VOTING_POWER"),
            string::utf8(b"PROPOSAL_ABILITY")
        ];
        ...
        move_to(&obj_signer,governance_token);
    }
    ...
    */
    inline fun remove_approved_hash(proposal_id: u64) acquires ApprovedExecutionHashes {
        ...
        let approved_exe_hash = borrow_global_mut<ApprovedExecutionHashes>(get_resource_account_address());
        if(simple_map::contains_key(&approved_exe_hash.hashes, &proposal_id)){
            simple_map::remove(&mut approved_exe_hash.hashes, &proposal_id);
        }else{
    ...
    */
    fun create_proposal_internal(
        ...
        //       duration
        let gov_config = borrow_global<GovernanceConfig>(proposer);
        let expiration_timestamp = timestamp::now_seconds() + gov_config.voting_duration_seconds;
        // TODO: Create the proposal in the aptos_framework::voting module
        //
        // HINT: Use voting::create_proposal_v2
        let gov_proposal_type = GovernanceProposalType{};
        voting::create_proposal_v2<GovernanceProposalType>(
            proposer,
            get_resource_account_address(),
            gov_proposal_type,
            execution_hash,
            gov_config.minimum_voting_threshold,
    ...
    */
    inline fun get_signer(key: String, description: String): signer acquires GovernanceResponsibility {
        ...
        let gov_resp = borrow_global<GovernanceResponsibility>(get_resource_account_address());
        // TODO: Generate the signer with the signer cap and return it
        let gov_key = GovernanceResponsibilityKey{key, description};
        account::create_signer_with_capability(simple_map::borrow(&gov_resp.signer_caps, &gov_key))
    }
    ...
    */
    inline fun generate_proposal_id_key(proposal_id: u64): String {
        ...
        let output = string::utf8(BASE_PROPOSAL_ID_KEY);
        string::append(&mut output, string_utils::to_string_with_integer_types(&proposal_id));
        output
    }
    ...
    */
    inline fun create_proposal_metadata(
        ...
        let new_map = simple_map::create();
        simple_map::add(&mut new_map, string::utf8(METADATA_LOCATION_KEY), metadata_location);
        simple_map::add(&mut new_map, string::utf8(METADATA_HASH_KEY), metadata_hash);
        new_map
    }
    ...
    */
    inline fun get_total_supply_of_governance_token(): Option<u64> {
        ...
        let token_obj = object::address_to_object<GovernanceToken>(get_governenace_token_address(@overmind));
    ...
    */
    inline fun generate_early_resolution_vote_threshold(): Option<u128> {
        ...
        let vote_thres= option::none<u128>();
        if(!option::is_none(&total_supply)){
            let num = ((*option::borrow(&total_supply) / 2 + 1) as u128);
            vote_thres = option::some(num);
        };
    ...
    */
    inline fun generate_governance_token_description(account_address: address): String {
        ...
        let output_string = string::utf8(BASE_TOKEN_DESCRIPTION);
        string::append(&mut output_string, string_utils::to_string_with_canonical_addresses(&account_address));
        output_string
    }
    ...
    */
    ...
    /*
    Self-added helper fun
    */
    inline fun get_governenace_token_address(account: address): address{
        token::create_token_address(
            &get_resource_account_address(),
            &string::utf8(GOVERNANCE_TOKEN_COLLECTION_NAME),
            &generate_governance_token_name(account)
        )
    }
    ...
    inline fun check_if_voter_has_not_voted_on_proposal(
        ...
        assert!(!property_map::contains_key(&governance_token, &proposal_id_key), EVoterAlreadyVotedOnProposal);
    }
    ...
    inline fun check_if_proposal_voting_is_open(proposal_id: u64) {
        ...
        assert!(!voting::is_voting_closed<GovernanceProposalType>(get_resource_account_address(),proposal_id), EProposalVotingIsClosed);
    }
    ...
    inline fun check_if_account_can_create_proposal(governance_token: Object<GovernanceToken>) {
        ...
        let check_bool = property_map::read_bool<GovernanceToken>(&governance_token,&string::utf8(b"PROPOSAL_ABILITY"));
        assert!(check_bool,EAccountDoesNotHaveProposalAbility);
    }
    ...
    inline fun check_if_account_is_resource_account_address(account_address: address) {
        ...
        assert!(account_address == get_resource_account_address(), EAccountIsNotResourceAccount);
    }
